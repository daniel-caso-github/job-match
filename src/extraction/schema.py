...
class JobRequirements(BaseModel):
    ...
    english_level: EnglishLevel | None = Field(
        default=None,
        description="Nivel de inglés requerido. Null si no se menciona.",
    )
    ...
    remote: bool | None = Field(
        default=None,
        description=(
            "True si es 100% remoto. False si presencial/híbrido obligatorio. "
    ...
    latam_friendly: bool | None = Field(
        default=None,
        description=(
            "True si menciona explícitamente que acepta LATAM o time zone Americas."
        ),
    )
    salary_range: str | None = Field(
        default=None,
        description="Rango salarial textual si la oferta lo publica.",
    )
    confidence: float = Field(
        ...
        le=1.0,
        description=(
            "Auto-evaluación del modelo sobre qué tan explícita fue la extracción. "
            "0=adivinanza, 1=explícito."
        ),
    )
